...
class OrderController extends Controller
{
    ...
    public function store(OrderStoreRequest $request)
    {
        ...
        $orderDate = $request->get('order_date'); // Access user-selected date

    if (!empty($orderDate)) {
        $formattedDate = Carbon::parse($orderDate)->format('Y-m-d');
    } else {
        // Handle case where no date is selected (optional)
        // You can set a default date or throw an error
        $formattedDate = Carbon::now()->format('Y-m-d'); // Fallback to current date
    }
